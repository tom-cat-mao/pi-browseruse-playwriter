...
`images: 'save'` fetches bytes for at most 20 images, each fetched by the page first so the request carries the page's cookies, and falling back to a runtime-side fetch when the page refuses (cross-origin without CORS headers, or the page's own CSP). Bytes are capped at 16 MiB per image and 64 MiB per request, and travel as `value.savedAssets` — never on the model-inline image channel. One image that cannot be fetched, is too large or is a type the artifact store cannot name is reported in `failedAssets` and never fails the rest of the extraction.

The relay writes every saved asset through the artifact store (`label` from the image `alt`, `sourceUrl` from its URL), returns the same artifact descriptors as a screenshot, rewrites the URLs of the stored images to their local artifact paths — in the persisted body and in the preview text the model reads — and strips the byte payload before the response leaves the runtime.
