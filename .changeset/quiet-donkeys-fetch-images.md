...
The relay writes every saved asset through the artifact store (`label` from the image `alt`, `sourceUrl` from its URL), returns the same artifact descriptors as a screenshot, rewrites every URL the page used for a stored image — the `src` attribute and the srcset candidate the browser chose — to its local artifact path, in the persisted body and in the preview text the model reads, and strips the byte payload before the response leaves the runtime.
