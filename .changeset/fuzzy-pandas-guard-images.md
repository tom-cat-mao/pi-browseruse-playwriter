...
On Chrome, a saved image now reports every URL the page used for it (`src` plus the chosen srcset candidate), and the relay rewrites all of them to the artifact path, so a Markdown body that kept the `src` attribute of an image whose bytes came from its srcset candidate no longer points at the remote URL. The Firefox backend rewrites the single URL it fetched.
